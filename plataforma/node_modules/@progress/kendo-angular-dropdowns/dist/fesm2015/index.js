...
class AutoCompleteComponent {
    ...
    set data(data) {
        ...
        const shouldSuggest = this.suggest && this.data.length && this.value;
        if (shouldSuggest) {
    ...
    search(text, startFrom = 0) {
        ...
        if (text.length && this.dataService.itemsCount) {
            const i = this.dataService.findIndex(this.findIndexPredicate(text), startFrom);
    ...
    subscribeEvents() {
        ...
        this.valueChangeSubscription = this.valueChangeSubject.pipe(filter$1((candidate) => {
        ...
        this.navigationSubscription = merge$1(this.navigationService.up, this.navigationService.down).subscribe((event) => this.navigate(event.index));
...
class ComboBoxComponent {
    constructor(localization, popupService, selectionService, navigationService, disabledItemsService, dataService, _zone, cdr, renderer, wrapper, touchEnabled) {
         ...
         * ```
         */
        this.valueNormalizer = (text) => text.pipe(map$1((userInput) => userInput));
        /**
    ...
    set text(text) {
        let textCandidate = this.prop(this.textField, true)(text);
    ...
    createValueStream() {
        ...
        const valueStream = this.valueSubject.pipe(filter$1((candidate) => {
        ...
        const customValueStreams = partition$1(() => this.allowCustom)(this.customValueSubject.pipe(throttleTime$1(300)));
        const allowCustomValueStream = customValueStreams[0].pipe(tap$1(() => {
            this.loading = true;
            this.disabled = true;
            this.cdr.detectChanges();
        }), filter$1(() => {
            ...
            return hasChange;
        }), this.valueNormalizer, map$1((normalizedValue) => {
            return {
                custom: true,
        ...
        const disableCustomValueStream = customValueStreams[1].pipe(map$1(() => {
            return {
                custom: true,
                dataItem: undefined,
                text: undefined,
        ...
        const clearValueStream = this.clearValueSubject.pipe(map$1(() => ({
            dataItem: undefined,
            text: undefined,
            value: undefined
        })));
        ...
        const merged = merge$1(valueStream, allowCustomValueStream, disableCustomValueStream, clearValueStream);
        this.valueSubscription = merged.pipe(catchError$1(() => {
    ...
    attachStreams() {
        ...
        this.subs.add(merge$1(this.navigationService.up, this.navigationService.down, this.navigationService.home, this.navigationService.end)
            .pipe(filter$1((event) => isPresent(event.index)))
        ...
        this.subs.add(this.navigationService.enter.pipe(tap$1((event) => {
            if (this.isOpen) {
                event.originalEvent.preventDefault();
            }
        }))
            .subscribe(this.handleEnter.bind(this)));
        this.subs.add(this.selectionSubject.pipe(distinctUntilChanged$1()).subscribe((dataItem) => {
            this.selectionChange.emit(dataItem);
        }));
        this.subs.add(merge$1(this.selectionService.onChange, this.selectionService.onSelect.pipe(filter$1(_ => !this.isOpen)))
            .pipe(tap$1(_ => {
            this._filtering = false;
            this.togglePopup(false);
        }), map$1((event) => this.dataService.itemAt(event.indices[0])))
            .subscribe(dataItem => {
            this.change(dataItem);
        }));
        this.subs.add(this.selectionService.onSelect.pipe(filter$1(_ => this.isOpen), tap$1(_ => this._filtering = false), map$1((event) => this.dataService.itemAt(event.indices[0])))
            .subscribe(dataItem => {
...
class DropDownListComponent {
    ...
    subscribeEvents() {
        ...
        this.selectionSubscription.add(this.selectionService.onSelect.pipe(filter$1(_ => this.isOpen), map$1(this.itemFromEvent.bind(this)))
            .subscribe(this.onSelectionChange.bind(this)));
        // item selection when the popup is closed | clicked | enter, etc.
        this.selectionSubscription.add(merge$1(this.selectionService.onSelect.pipe(filter$1(_ => !this.isOpen)), this.selectionService.onChange).pipe(map$1(this.itemFromEvent.bind(this)), tap$1(_ => this.togglePopup(false)))
        ...
        this.navigationSubscription = merge$1(this.navigationService.up, this.navigationService.down, this.navigationService.left.pipe(skipWhile$1(() => this.filterable)), this.navigationService.right.pipe(skipWhile$1(() => this.filterable)), this.navigationService.home, this.navigationService.end)
            .pipe(filter$1((event) => !isNaN(event.index)))
        ...
        this.filterBlurredSubscription = this.filterBlurred.pipe(concatMap$1(() => interval$1(10).pipe(take$1(1), takeUntil$1(this.wrapperFocused))))
            .subscribe(() => {
            this.wrapperBlurred.emit();
        });
        this._zone.runOutsideAngular(() => {
            this.componentBlurredSubscription =
                merge$1(this.wrapperBlurred.pipe(concatMap$1(() => interval$1(10).pipe(take$1(1), takeUntil$1(this.filterFocused)))), this.navigationService.tab).pipe(tap$1(event => event instanceof NavigationEvent && this.focus()), filter$1(() => this.isFocused))
...
class MultiSelectComponent {
    constructor(localization, popupService, dataService, selectionService, navigationService, disabledItemsService, cdr, differs, renderer, hostElement, _zone, touchEnabled) {
         ...
         * ```
         */
        this.valueNormalizer = (text) => text.pipe(map$1((userInput) => {
    ...
    subscribeEvents() {
        ...
        [
            ...
            this.navigationService.up.pipe(filter$1(isOpen)).subscribe((event) => this.handleUp(event.index)),
            this.navigationService.home.pipe(filter$1(() => isClosed)).subscribe(this.handleHome.bind(this)),
            this.navigationService.end.pipe(filter$1(() => isClosed)).subscribe(this.handleEnd.bind(this)),
    ...
    removeGroupTag(dataItems) {
        ...
        selectedIndices(dataItems, data, this.valueField)
            .forEach((currentValue) => this.selectionService.unselect(currentValue));
        dataItems
            .filter(dataItem => !isPresent(data.find(value => prop(value) === prop(dataItem))))
            .forEach(dataItem => {
            const filter$$1 = item => prop(item) !== prop(dataItem);
            this.value = this.value.filter(filter$$1);
            this.selectedDataItems = this.selectedDataItems.filter(filter$$1);
    ...
    createCustomValueStream() {
        ...
        this.customValueSubscription = this.customValueSubject.pipe(tap$1(() => {
            this.loading = true;
            this.disabled = true;
            this.cdr.detectChanges();
        }), this.valueNormalizer, catchError$1(() => {
            this.loading = false;
            this.disabled = false;
            this.clearFilter();
            if (this.autoClose) {
    ...
    searchTextAndFocus(text) {
        const index = this.allowCustom ? -1 : this.findIndex(text);
        this.selectionService.focused = index;
    }
    closePopup() {
        this.togglePopup(false);
...
class ListComponent {
    /* tslint:disable:member-ordering */
    constructor(wrapper, dataService, selectionService, disabledItemsService) {
        ...
        this.selectSubscription = this.selectionService
            .onSelect.pipe(map$1((args) => args.indices[0]), merge$3(this.selectionService.onFocus))
...
/** @nocollapse */
SharedDirectivesModule.ctorParameters = () => [];
...
/** @nocollapse */
DropDownsModule.ctorParameters = () => [];
