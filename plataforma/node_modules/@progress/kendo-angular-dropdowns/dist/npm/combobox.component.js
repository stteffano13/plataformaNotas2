...
var partition_1 = require("rxjs/operators/partition");
var tap_1 = require("rxjs/operators/tap");
var of_1 = require("rxjs/observable/of");
var throttleTime_1 = require("rxjs/operators/throttleTime");
...
var ComboBoxComponent = /** @class */ (function () {
    function ComboBoxComponent(localization, popupService, selectionService, navigationService, disabledItemsService, dataService, _zone, cdr, renderer, wrapper, touchEnabled) {
        ...
        this.valueNormalizer = function (text) {
            return text.pipe(map_1.map(function (userInput) { return userInput; }));
        };
        /**
        ...
        this.clearValueSubject = new Subject_1.Subject();
    ...
    ComboBoxComponent.prototype.createValueStream = function () {
        ...
        var valueStream = this.valueSubject.pipe(filter_1.filter(function (candidate) {
        ...
        var customValueStreams = partition_1.partition(function () { return _this.allowCustom; })(this.customValueSubject.pipe(throttleTime_1.throttleTime(300)));
        var allowCustomValueStream = customValueStreams[0].pipe(tap_1.tap(function () {
            _this.loading = true;
            _this.disabled = true;
            ...
            return hasChange;
        }), this.valueNormalizer, map_1.map(function (normalizedValue) {
            return {
                custom: true,
        ...
        var disableCustomValueStream = customValueStreams[1].pipe(map_1.map(function () {
            return {
                custom: true,
                dataItem: undefined,
                text: undefined,
        ...
        var clearValueStream = this.clearValueSubject.pipe(map_1.map(function () { return ({
            dataItem: undefined,
            text: undefined,
            value: undefined
        }); }));
        ...
        var merged = merge_1.merge(valueStream, allowCustomValueStream, disableCustomValueStream, clearValueStream);
        this.valueSubscription = merged.pipe(catchError_1.catchError(function () {
    ...
    ComboBoxComponent.prototype.attachStreams = function () {
        ...
        this.subs.add(merge_1.merge(this.navigationService.up, this.navigationService.down, this.navigationService.home, this.navigationService.end)
            .pipe(filter_1.filter(function (event) { return util_1.isPresent(event.index); }))
        ...
        this.subs.add(this.navigationService.enter.pipe(tap_1.tap(function (event) {
            if (_this.isOpen) {
                event.originalEvent.preventDefault();
            }
        }))
            .subscribe(this.handleEnter.bind(this)));
        this.subs.add(this.selectionSubject.pipe(distinctUntilChanged_1.distinctUntilChanged()).subscribe(function (dataItem) {
            _this.selectionChange.emit(dataItem);
        }));
        this.subs.add(merge_1.merge(this.selectionService.onChange, this.selectionService.onSelect.pipe(filter_1.filter(function (_) { return !_this.isOpen; })))
            .pipe(tap_1.tap(function (_) {
            _this._filtering = false;
            _this.togglePopup(false);
        }), map_1.map(function (event) { return _this.dataService.itemAt(event.indices[0]); }))
            .subscribe(function (dataItem) {
            _this.change(dataItem);
        }));
        this.subs.add(this.selectionService.onSelect.pipe(filter_1.filter(function (_) { return _this.isOpen; }), tap_1.tap(function (_) { return _this._filtering = false; }), map_1.map(function (event) { return _this.dataService.itemAt(event.indices[0]); }))
            .subscribe(function (dataItem) {
